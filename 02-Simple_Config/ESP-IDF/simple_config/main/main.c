...
static const char *TAG = "simple_config";
...
void save_configs()
{
    // save configs from EEPROM or where ever
}

void load_configs()
{
    // load configs from EEPROM or where ever
}
...
void otadrive_thread(void *pvParameter)
{
    while (1)
    {
        if (otadrive_timeTick(30))
        {
            ...
            ESP_LOGI(TAG, "Check new OTA result %d,%lu", r.code, r.size);
